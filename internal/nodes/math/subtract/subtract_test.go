...
func module(t *testing.T) nodes.Node {
	t.Helper()
	registry := nodes.New()
	if err := subtract.Register(registry); err != nil {
		t.Fatalf("Register() error = %v", err)
	}
	registered, ok := registry.Get("math:subtract")
	if !ok {
		t.Fatal("math:subtract was not registered")
	}
	return registered
}
...
func invocation(inputs map[string]any) nodes.Invocation {
	return nodes.Invocation{
		Node:            domain.FlowNode{ID: "sub-1", Type: "math:subtract", Data: map[string]any{"config": map[string]any{}}},
...
func TestRegisterMetadata(t *testing.T) {
	...
	if definition.Type != "math:subtract" || definition.Mode != domain.NodePure || definition.Category != "Math" {
...
func TestExecuteSubtractsBFromA(t *testing.T) {
	tests := []struct {
		name string
		a, b any
		want float64
	}{
		{"floats", float64(5), float64(3), 2},
		{"int inputs", 10, 4, 6},
		{"negative result", float64(2.5), float64(5), -2.5},
...
func TestExecuteRejectsInvalidInputs(t *testing.T) {
	tests := []struct {
		...
		{"text b", map[string]any{"a": float64(1), "b": "1"}},
...
func TestExecuteRejectsNonFiniteResult(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Execute() accepted an overflowing difference")
	}
}
