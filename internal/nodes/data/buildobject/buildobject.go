...
func Register(registrar nodes.Registrar) error {
	...
	definition := datanodes.Node("data:build_object", "Data", "Build Object", "Build an object from configurable typed input pins.", "braces", nil, []domain.NodePort{datanodes.Pin("object", "Object", domain.PinOutput, domain.DataObject)}, []domain.ConfigField{datanodes.ObjectFields("fields", "Fields")}, map[string]any{"fields": defaults})
...
func ResolveDefinition(definition domain.NodeDefinition, node domain.FlowNode) (domain.NodeDefinition, error) {
	fields, err := datanodes.ObjectFieldsFor(config(node), definition.DefaultConfig)
	if err != nil {
