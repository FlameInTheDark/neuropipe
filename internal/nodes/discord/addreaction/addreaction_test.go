...
type senderStub struct {
	request domain.DiscordReactionRequest
	result  domain.DiscordActionResult
	err     error
}

func (s *senderStub) AddDiscordReaction(_ context.Context, request domain.DiscordReactionRequest) (domain.DiscordActionResult, error) {
	s.request = request
...
func registeredModule(t *testing.T) nodes.Node {
	...
	module, ok := registry.Get("action:discord_add_reaction")
	if !ok {
		t.Fatal("action:discord_add_reaction was not registered")
	}
	return module
}
...
func invocation(definition domain.NodeDefinition, inputs map[string]any) nodes.Invocation {
	return nodes.Invocation{
		Node:            domain.FlowNode{Type: "action:discord_add_reaction", Data: map[string]any{"config": map[string]any{}}},
...
func TestRegistrationMetadata(t *testing.T) {
	...
	if definition.Type != "action:discord_add_reaction" || definition.Mode != domain.NodeImpure || definition.Category != "Discord" {
		t.Fatalf("definition = %#v", definition)
	}
	assertPinIDs(t, definition.Inputs, []string{"in", "channel", "messageId", "emoji", "identityId"})
	assertPinIDs(t, definition.Outputs, []string{"done", "rejected", "reason"})
}

func TestReactsAndFollowsDonePort(t *testing.T) {
	...
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": "55", "messageId": "77", "emoji": "👍", "identityId": "bot-1",
	...
	if sender.request.ChannelID != "55" || sender.request.MessageID != "77" || sender.request.Emoji != "👍" || sender.request.IdentityID != "bot-1" {
		t.Fatalf("request = %#v", sender.request)
	}
}

func TestSoftRejectionFollowsRejectedPort(t *testing.T) {
	sender := &senderStub{result: domain.DiscordActionResult{Reason: "Unknown Emoji"}}
	module := registeredModule(t)
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": "55", "messageId": "77", "emoji": "👍",
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Ports) != 1 || result.Ports[0] != "rejected" || result.Outputs["reason"] != "Unknown Emoji" {
...
func TestValidationErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing channel", map[string]any{"messageId": "77", "emoji": "👍"}, runtimeStub{sender: &senderStub{}}, "channel ID is required"},
		{"missing message id", map[string]any{"channel": "55", "emoji": "👍"}, runtimeStub{sender: &senderStub{}}, "message ID is required"},
		{"missing emoji", map[string]any{"channel": "55", "messageId": "77"}, runtimeStub{sender: &senderStub{}}, "emoji is required"},
		{"nil runtime", map[string]any{"channel": "55", "messageId": "77", "emoji": "👍"}, nil, "discord delivery is unavailable"},
		{"runtime without provider", map[string]any{"channel": "55", "messageId": "77", "emoji": "👍"}, struct{}{}, "discord delivery is unavailable"},
		{"provider with nil sender", map[string]any{"channel": "55", "messageId": "77", "emoji": "👍"}, runtimeStub{}, "discord delivery is unavailable"},
	}
...
// Channel and message IDs frequently arrive as numbers (numeric constants,
// JSON extraction); they must be coerced to digit strings instead of being
// silently dropped, and string emoji input is trimmed.
func TestNumericIDInputsAreCoerced(t *testing.T) {
	sender := &senderStub{result: domain.DiscordActionResult{Done: true}}
	module := registeredModule(t)
	_, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": json.Number("565062979255795712"), "messageId": float64(77), "emoji": " 👍 ",
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if sender.request.ChannelID != "565062979255795712" || sender.request.MessageID != "77" || sender.request.Emoji != "👍" {
		t.Fatalf("request = %#v", sender.request)
	}
}
