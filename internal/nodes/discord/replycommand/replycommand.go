// Package replycommand registers the Reply to Command action node: it
// answers one application-command interaction sent by the Command Trigger.
...
func definition() domain.NodeDefinition {
	return dc.Definition("action:discord_reply_command", "Reply to Command", "Answer the application command that started this pipeline: replaces the deferred loading state, or sends the initial reply within 3 seconds in manual mode.", "reply",
		[]domain.NodePort{
			dc.Exec("in", "Reply", domain.PinInput),
			interactionPort(),
		...
		[]domain.ConfigField{
			...
			{Name: "ephemeral", Label: "Ephemeral (only the invoking member sees the reply)", Kind: "boolean"},
		},
		map[string]any{
			"message": "", "embeds": sendmessage.EmptyEmbedDocument(), "embedsJson": "", "ephemeral": false,
		},
	)
}

// interactionPort is the handoff pin: it accepts the Command Trigger's
// Interaction output and carries the interaction token the response
// endpoints need.
...
func resolve(node domain.FlowNode) (domain.NodeDefinition, error) {
	...
	resolved.Inputs = inputsWithDocument
	return resolved, nil
}
...
func execute(ctx context.Context, invocation nodes.Invocation, runtime nodes.Runtime) (nodes.ExecutionResult, error) {
	...
	result, err := sender.RespondDiscordInteraction(ctx, domain.DiscordCommandReplyRequest{
		IdentityID:  dc.String(invocation, "identityId"),
		Interaction: reference,
		Message:     message,
		Embeds:      embeds,
		Ephemeral:   dc.BoolValue(invocation, "ephemeral"),
	})
	if err != nil {
		return nodes.ExecutionResult{}, err
	}
...
// resolveEmbeds mirrors the Send Message node: a non-empty embedsJson pin
// overrides the editor document, whose {{templates}} resolve from input
// pins.
