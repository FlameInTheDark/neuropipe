...
type senderStub struct {
	request domain.DiscordDeleteRequest
	result  domain.DiscordActionResult
	err     error
}

func (s *senderStub) DeleteDiscordMessage(_ context.Context, request domain.DiscordDeleteRequest) (domain.DiscordActionResult, error) {
	s.request = request
...
func registeredModule(t *testing.T) nodes.Node {
	...
	module, ok := registry.Get("action:discord_delete_message")
	if !ok {
		t.Fatal("action:discord_delete_message was not registered")
	}
	return module
}
...
func invocation(definition domain.NodeDefinition, inputs map[string]any) nodes.Invocation {
	return nodes.Invocation{
		Node:            domain.FlowNode{Type: "action:discord_delete_message", Data: map[string]any{"config": map[string]any{}}},
...
func TestRegistrationMetadata(t *testing.T) {
	...
	if definition.Type != "action:discord_delete_message" || definition.Mode != domain.NodeImpure || definition.Category != "Discord" {
		t.Fatalf("definition = %#v", definition)
	}
	assertPinIDs(t, definition.Inputs, []string{"in", "channel", "messageId", "identityId"})
...
func TestDeletesMessageAndFollowsDonePort(t *testing.T) {
	...
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": "55", "messageId": "77", "identityId": "bot-1",
	...
	if sender.request.ChannelID != "55" || sender.request.MessageID != "77" || sender.request.IdentityID != "bot-1" {
		t.Fatalf("request = %#v", sender.request)
	}
}

func TestSoftRejectionFollowsRejectedPort(t *testing.T) {
	sender := &senderStub{result: domain.DiscordActionResult{Reason: "Missing Permissions"}}
	module := registeredModule(t)
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": "55", "messageId": "77",
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidationErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing channel", map[string]any{"messageId": "77"}, runtimeStub{sender: &senderStub{}}, "channel ID is required"},
		{"missing message id", map[string]any{"channel": "55"}, runtimeStub{sender: &senderStub{}}, "message ID is required"},
		{"nil runtime", map[string]any{"channel": "55", "messageId": "77"}, nil, "discord delivery is unavailable"},
		{"runtime without provider", map[string]any{"channel": "55", "messageId": "77"}, struct{}{}, "discord delivery is unavailable"},
		{"provider with nil sender", map[string]any{"channel": "55", "messageId": "77"}, runtimeStub{}, "discord delivery is unavailable"},
	}
...
func TestNumericIDInputsAreCoerced(t *testing.T) {
	...
	module := registeredModule(t)
	_, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"channel": json.Number("565062979255795712"), "messageId": 77,
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if sender.request.ChannelID != "565062979255795712" || sender.request.MessageID != "77" {
		t.Fatalf("request = %#v", sender.request)
	}
}
