// Package tg provides shared pin, field, and runtime helpers for first-party
// Telegram node modules. It contains no execution logic of its own so every
...
// Exec builds the control-flow pin used by every impure Telegram node.
...
func Text(id, label string, direction domain.PinDirection, required bool) domain.NodePort {
	return domain.NodePort{ID: id, Label: label, Kind: domain.PinData, Direction: direction, DataType: domain.DataText, Type: &textType, Color: "#229ed9", Required: required, MaxConnections: 1}
}
...
// IdentityField is the bot-identity picker shared by every Telegram node.
func IdentityField() domain.ConfigField {
	return domain.ConfigField{Name: "identityId", Label: "Bot identity", Kind: "telegram-identity"}
}

// Definition assembles the common NodeDefinition skeleton for Telegram nodes.
func Definition(nodeType, label, description, icon string, inputs []domain.NodePort, outputs []domain.NodePort, fields []domain.ConfigField, defaults map[string]any) domain.NodeDefinition {
	...
	return domain.NodeDefinition{
		Type: nodeType, Category: "Telegram", Label: label, Description: description,
		Icon: icon, Color: "#229ed9", Mode: domain.NodeImpure, PortContractOwned: true,
...
// Sender resolves the Telegram capability from the runtime. Nodes never
// import the polling service; they only pass typed requests.
func Sender(runtime nodes.Runtime) (nodes.TelegramSender, error) {
	provider, ok := runtime.(nodes.TelegramSenderProvider)
	if !ok || provider.TelegramSender() == nil {
		return nil, fmt.Errorf("telegram delivery is unavailable")
	}
	return provider.TelegramSender(), nil
}

// String reads one string pin value (wired or config fallback). Numeric
// values are coerced because entity IDs (chats, messages, users) are
