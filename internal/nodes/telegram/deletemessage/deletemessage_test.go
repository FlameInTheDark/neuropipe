...
type senderStub struct {
	request domain.TelegramDeleteRequest
...
func registeredModule(t *testing.T) nodes.Node {
	...
	module, ok := registry.Get("action:telegram_delete_message")
	if !ok {
		t.Fatal("action:telegram_delete_message was not registered")
	}
	return module
}
...
func invocation(definition domain.NodeDefinition, inputs map[string]any) nodes.Invocation {
	return nodes.Invocation{
		Node:            domain.FlowNode{Type: "action:telegram_delete_message", Data: map[string]any{"config": map[string]any{}}},
...
func TestRegistrationMetadata(t *testing.T) {
	...
	if definition.Type != "action:telegram_delete_message" || definition.Mode != domain.NodeImpure || definition.Category != "Telegram" {
		t.Fatalf("definition = %#v", definition)
	}
	assertPinIDs(t, definition.Inputs, []string{"in", "chatId", "messageId", "identityId"})
	assertPinIDs(t, definition.Outputs, []string{"done", "rejected", "reason"})
}

func TestDeletesMessageAndFollowsDonePort(t *testing.T) {
	...
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"chatId": "55", "messageId": "321", "identityId": "bot-1",
	...
	if sender.request.ChatID != "55" || sender.request.MessageID != "321" || sender.request.IdentityID != "bot-1" {
		t.Fatalf("request = %#v", sender.request)
	}
}

func TestSoftRejectionFollowsRejectedPort(t *testing.T) {
	sender := &senderStub{result: domain.TelegramActionResult{Reason: "message to delete not found"}}
	module := registeredModule(t)
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"chatId": "55", "messageId": "321",
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Ports) != 1 || result.Ports[0] != "rejected" || result.Outputs["reason"] != "message to delete not found" {
...
func TestValidationErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing chat id", map[string]any{"messageId": "321"}, runtimeStub{sender: &senderStub{}}, "chat ID is required"},
		{"missing message id", map[string]any{"chatId": "55"}, runtimeStub{sender: &senderStub{}}, "message ID is required"},
		{"nil runtime", map[string]any{"chatId": "55", "messageId": "321"}, nil, "telegram delivery is unavailable"},
		{"runtime without provider", map[string]any{"chatId": "55", "messageId": "321"}, struct{}{}, "telegram delivery is unavailable"},
		{"provider with nil sender", map[string]any{"chatId": "55", "messageId": "321"}, runtimeStub{}, "telegram delivery is unavailable"},
	}
...
// Chat and message IDs frequently arrive as numbers (numeric constants, JSON
// extraction); they must be coerced to digit strings instead of being
// silently dropped.
func TestNumericIDInputsAreCoerced(t *testing.T) {
	sender := &senderStub{result: domain.TelegramActionResult{Done: true}}
	module := registeredModule(t)
	_, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"chatId": json.Number("-1001234567890"), "messageId": float64(321),
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if sender.request.ChatID != "-1001234567890" || sender.request.MessageID != "321" {
		t.Fatalf("request = %#v", sender.request)
	}
}
