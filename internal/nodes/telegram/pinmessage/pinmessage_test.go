...
type senderStub struct {
	request domain.TelegramPinRequest
...
func registeredModule(t *testing.T) nodes.Node {
	...
	module, ok := registry.Get("action:telegram_pin_message")
	if !ok {
		t.Fatal("action:telegram_pin_message was not registered")
	}
	return module
}
...
func invocation(definition domain.NodeDefinition, inputs map[string]any) nodes.Invocation {
	return nodes.Invocation{
		Node:            domain.FlowNode{Type: "action:telegram_pin_message", Data: map[string]any{"config": map[string]any{}}},
...
func TestRegistrationMetadata(t *testing.T) {
	...
	if definition.Type != "action:telegram_pin_message" || definition.Mode != domain.NodeImpure || definition.Category != "Telegram" {
		t.Fatalf("definition = %#v", definition)
	}
	assertPinIDs(t, definition.Inputs, []string{"in", "chatId", "messageId", "notify", "unpin", "identityId"})
	assertPinIDs(t, definition.Outputs, []string{"done", "rejected", "reason"})
}

func TestPinsMessageWithNotification(t *testing.T) {
	...
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"chatId": "55", "messageId": "321", "notify": true, "identityId": "bot-1",
	...
	if sender.request.ChatID != "55" || sender.request.MessageID != "321" || !sender.request.Notify || sender.request.Unpin || sender.request.IdentityID != "bot-1" {
		t.Fatalf("request = %#v", sender.request)
	}
}

// unpin routes through the same sender call; the request must carry both
// flags exactly as wired.
func TestUnpinFlagForwarded(t *testing.T) {
	sender := &senderStub{result: domain.TelegramActionResult{Done: true}}
	module := registeredModule(t)
	_, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
		"chatId": "55", "messageId": "321", "unpin": true,
	}), runtimeStub{sender: sender})
	if err != nil {
		t.Fatal(err)
	}
	if sender.request.Notify || !sender.request.Unpin {
		t.Fatalf("request = %#v", sender.request)
	}
}

func TestSoftRejectionFollowsRejectedPort(t *testing.T) {
	sender := &senderStub{result: domain.TelegramActionResult{Reason: "not enough rights"}}
	module := registeredModule(t)
	result, err := module.Execute(context.Background(), invocation(module.Definition(), map[string]any{
	...
	if len(result.Ports) != 1 || result.Ports[0] != "rejected" || result.Outputs["reason"] != "not enough rights" {
