// Package renamefolder registers the Rename Folder Blueprint node.
package renamefolder
...
func definition() domain.NodeDefinition {
	...
	return domain.NodeDefinition{
		Type:        "action:rename_folder",
		Category:    "Folders",
		Label:       "Rename Folder",
		Description: "Rename a folder within the same parent directory. The new name must be a single path segment.",
		Icon:        "pencil-line",
		Color:       "#c4b5fd",
		...
		Fields: []domain.ConfigField{
			{Name: "path", Label: "Path", Kind: "string", Placeholder: "C:\\Work\\old", Required: true},
			{Name: "newName", Label: "New name", Kind: "string", Placeholder: "new", Required: true},
		},
...
func execute(_ context.Context, invocation nodes.Invocation, _ nodes.Runtime) (nodes.ExecutionResult, error) {
	...
	if err != nil {
		return nodes.ExecutionResult{}, fmt.Errorf("rename folder: %w", err)
	}
	...
	if err := fileops.ValidateName(newName); err != nil {
		return nodes.ExecutionResult{}, fmt.Errorf("rename folder new name: %w", err)
	}
	// Reject renaming into itself or a nested child path.
	cleanPath := filepath.Clean(path)
	parent := filepath.Dir(cleanPath)
	newPath := filepath.Join(parent, newName)
	if newPath == cleanPath {
		return nodes.ExecutionResult{}, fmt.Errorf("new name is the same as the current name")
	}
	if exists, err := fileops.PathExists(newPath); err != nil {
		return nodes.ExecutionResult{}, fmt.Errorf("check target: %w", err)
	} else if exists {
		return nodes.ExecutionResult{}, fmt.Errorf("a folder with name %q already exists in the same directory", newName)
	}
