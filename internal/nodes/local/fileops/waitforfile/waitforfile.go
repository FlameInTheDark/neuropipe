// Package waitforfile registers the Wait For File Blueprint node.
package waitforfile
...
func definition() domain.NodeDefinition {
	...
	return domain.NodeDefinition{
		Type:        "action:wait_for_file",
		Category:    "Files",
		Label:       "Wait For File",
		Description: "Poll a path until a regular file exists at it, or until the timeout elapses.",
		Icon:        "hourglass",
		Color:       "#c4b5fd",
		...
		Fields: []domain.ConfigField{
			{Name: "path", Label: "Path", Kind: "string", Placeholder: "C:\\Work\\ready.txt", Required: true},
...
func execute(ctx context.Context, invocation nodes.Invocation, _ nodes.Runtime) (nodes.ExecutionResult, error) {
	...
	if err != nil {
		return nodes.ExecutionResult{}, fmt.Errorf("wait for file: %w", err)
	}
	timeout := floatValue(invocation, "timeoutSeconds", 30)
	if timeout <= 0 {
		return nodes.ExecutionResult{}, fmt.Errorf("wait for file timeout must be greater than zero")
	}
	...
	for {
		if err := ctx.Err(); err != nil {
			return nodes.ExecutionResult{}, fmt.Errorf("wait for file cancelled: %w", err)
		}
		...
		select {
		case <-ctx.Done():
			return nodes.ExecutionResult{}, fmt.Errorf("wait for file cancelled: %w", ctx.Err())
...
// filepath import retained because future versions may expose path normalization
// helpers that surface the cleaned path on the result; today we only clean.
