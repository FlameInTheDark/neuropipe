...
func (s *Store) RecordMetricExecution(ctx context.Context, execution domain.Execution) error {
	...
	if err := statements(s.db).Select("name").From("pipelines").Where(squirrel.Eq{"id": execution.PipelineID}).QueryRowContext(ctx).Scan(&pipelineName); err != nil && err != sql.ErrNoRows {
	...
	if _, err := statements(tx).Replace("metric_execution_events").Columns("execution_id", "pipeline_id", "pipeline_name", "trigger_kind", "status", "occurred_at", "duration_ms", "queue_wait_ms", "node_count", "failed_node_count").Values(event.ExecutionID, event.PipelineID, event.PipelineName, event.TriggerKind, event.Status, stamp(event.OccurredAt), event.DurationMS, event.QueueWaitMS, event.NodeCount, event.FailedNodeCount).ExecContext(ctx); err != nil {
		return fmt.Errorf("insert execution metric: %w", err)
	}
	if _, err := statements(tx).Delete("metric_node_events").Where(squirrel.Eq{"execution_id": event.ExecutionID}).ExecContext(ctx); err != nil {
	...
	for _, run := range execution.NodeRuns {
		...
		if _, err := statements(tx).Insert("metric_node_events").Columns("execution_id", "pipeline_id", "node_type", "status", "occurred_at", "duration_ms").Values(event.ExecutionID, event.PipelineID, run.NodeType, run.Status, stamp(event.OccurredAt), durationMS).ExecContext(ctx); err != nil {
...
func (s *Store) metricTriggerKind(ctx context.Context, triggerID string) domain.TriggerKind {
	...
	var kind string
	if err := statements(s.db).Select("kind").From("trigger_bindings").Where(squirrel.Eq{"id": triggerID}).QueryRowContext(ctx).Scan(&kind); err == nil {
...
func (s *Store) RecordMetricActivity(ctx context.Context, event domain.MetricActivityEvent) error {
	...
	_, err := statements(s.db).Insert("metric_activity_events").Columns("id", "kind", "outcome", "duration_ms", "occurred_at").Values(uuid.NewString(), event.Kind, event.Outcome, event.DurationMS, stamp(event.OccurredAt)).ExecContext(ctx)
	if err != nil {
...
func (s *Store) RecordMetricResource(ctx context.Context, point domain.MetricsResourcePoint) error {
	...
	_, err := statements(s.db).Insert("metric_resource_samples").Columns("id", "process_name", "cpu_percent", "working_set_bytes", "occurred_at").Values(uuid.NewString(), point.Process, point.CPUPercent, point.WorkingSet, stamp(point.At)).ExecContext(ctx)
	if err != nil {
...
func (s *Store) PurgeMetrics(ctx context.Context, settings domain.MetricsSettings) error {
	...
	rollups := []squirrel.InsertBuilder{
		statements(tx).Insert("metric_execution_rollups").Columns("bucket", "pipeline_id", "pipeline_name", "trigger_kind", "status", "run_count", "duration_sum_ms", "queue_wait_sum_ms", "node_count", "failed_node_count").Select(sqliteStatements.Select("substr(occurred_at, 1, 10)", "pipeline_id", "pipeline_name", "trigger_kind", "status", "COUNT(*)", "SUM(duration_ms)", "SUM(queue_wait_ms)", "SUM(node_count)", "SUM(failed_node_count)").From("metric_execution_events").Where("occurred_at < ?", stampCutoff).GroupBy("substr(occurred_at, 1, 10)", "pipeline_id", "pipeline_name", "trigger_kind", "status")).Suffix("ON CONFLICT(bucket, pipeline_id, trigger_kind, status) DO UPDATE SET run_count = run_count + excluded.run_count, duration_sum_ms = duration_sum_ms + excluded.duration_sum_ms, queue_wait_sum_ms = queue_wait_sum_ms + excluded.queue_wait_sum_ms, node_count = node_count + excluded.node_count, failed_node_count = failed_node_count + excluded.failed_node_count"),
		statements(tx).Insert("metric_llm_rollups").Columns("bucket", "provider_id", "provider_name", "provider_kind", "model", "succeeded", "call_count", "prompt_tokens", "completion_tokens", "tokens_reported_count", "duration_sum_ms", "queue_wait_sum_ms", "estimated_cost_sum_usd", "priced_count").Select(sqliteStatements.Select("substr(occurred_at, 1, 10)", "provider_id", "provider_name", "provider_kind", "model", "succeeded", "COUNT(*)", "SUM(prompt_tokens)", "SUM(completion_tokens)", "SUM(tokens_reported)", "SUM(duration_ms)", "SUM(queue_wait_ms)", "SUM(COALESCE(estimated_cost_usd, 0))", "SUM(CASE WHEN estimated_cost_usd IS NULL THEN 0 ELSE 1 END)").From("metric_llm_events").Where("occurred_at < ?", stampCutoff).GroupBy("substr(occurred_at, 1, 10)", "provider_id", "provider_name", "provider_kind", "model", "succeeded")).Suffix("ON CONFLICT(bucket, provider_id, model, succeeded) DO UPDATE SET call_count = call_count + excluded.call_count, prompt_tokens = prompt_tokens + excluded.prompt_tokens, completion_tokens = completion_tokens + excluded.completion_tokens, tokens_reported_count = tokens_reported_count + excluded.tokens_reported_count, duration_sum_ms = duration_sum_ms + excluded.duration_sum_ms, queue_wait_sum_ms = queue_wait_sum_ms + excluded.queue_wait_sum_ms, estimated_cost_sum_usd = estimated_cost_sum_usd + excluded.estimated_cost_sum_usd, priced_count = priced_count + excluded.priced_count"),
		statements(tx).Insert("metric_activity_rollups").Columns("bucket", "kind", "outcome", "event_count", "duration_sum_ms").Select(sqliteStatements.Select("substr(occurred_at, 1, 10)", "kind", "outcome", "COUNT(*)", "SUM(duration_ms)").From("metric_activity_events").Where("occurred_at < ?", stampCutoff).GroupBy("substr(occurred_at, 1, 10)", "kind", "outcome")).Suffix("ON CONFLICT(bucket, kind, outcome) DO UPDATE SET event_count = event_count + excluded.event_count, duration_sum_ms = duration_sum_ms + excluded.duration_sum_ms"),
		statements(tx).Insert("metric_resource_rollups").Columns("bucket", "process_name", "sample_count", "cpu_sum", "cpu_peak", "working_set_sum", "working_set_peak").Select(sqliteStatements.Select("substr(occurred_at, 1, 10)", "process_name", "COUNT(*)", "SUM(cpu_percent)", "MAX(cpu_percent)", "SUM(working_set_bytes)", "MAX(working_set_bytes)").From("metric_resource_samples").Where("occurred_at < ?", stampCutoff).GroupBy("substr(occurred_at, 1, 10)", "process_name")).Suffix("ON CONFLICT(bucket, process_name) DO UPDATE SET sample_count = sample_count + excluded.sample_count, cpu_sum = cpu_sum + excluded.cpu_sum, cpu_peak = MAX(cpu_peak, excluded.cpu_peak), working_set_sum = working_set_sum + excluded.working_set_sum, working_set_peak = MAX(working_set_peak, excluded.working_set_peak)"),
	}
	for _, rollup := range rollups {
		if _, err := rollup.ExecContext(ctx); err != nil {
	...
	for _, table := range []string{"metric_execution_events", "metric_node_events", "metric_llm_events", "metric_activity_events", "metric_resource_samples"} {
		if _, err := statements(tx).Delete(table).Where("occurred_at < ?", stampCutoff).ExecContext(ctx); err != nil {
	...
	for _, table := range []string{"metric_execution_rollups", "metric_llm_rollups", "metric_activity_rollups", "metric_resource_rollups"} {
		if _, err := statements(tx).Delete(table).Where("bucket < ?", rollupCutoff).ExecContext(ctx); err != nil {
			return fmt.Errorf("purge %s: %w", table, err)
		}
	}
	if err := tx.Commit(); err != nil {
...
func (s *Store) ClearMetrics(ctx context.Context) error {
	for _, table := range []string{"metric_execution_events", "metric_node_events", "metric_llm_events", "metric_activity_events", "metric_resource_samples", "metric_execution_rollups", "metric_llm_rollups", "metric_activity_rollups", "metric_resource_rollups"} {
		if _, err := statements(s.db).Delete(table).ExecContext(ctx); err != nil {
...
func (s *Store) readMetricExecutions(ctx context.Context, from, to time.Time, filter domain.MetricsFilter) ([]domain.MetricExecutionEvent, error) {
	rows, err := statements(s.db).Select("execution_id", "pipeline_id", "pipeline_name", "trigger_kind", "status", "occurred_at", "duration_ms", "queue_wait_ms", "node_count", "failed_node_count").From("metric_execution_events").Where("occurred_at >= ?", stamp(from)).Where("occurred_at <= ?", stamp(to)).OrderBy("occurred_at").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricNodes(ctx context.Context, from, to time.Time, filter domain.MetricsFilter) ([]domain.MetricNodeEvent, error) {
	rows, err := statements(s.db).Select("execution_id", "pipeline_id", "node_type", "status", "occurred_at", "duration_ms").From("metric_node_events").Where("occurred_at >= ?", stamp(from)).Where("occurred_at <= ?", stamp(to)).OrderBy("occurred_at").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricLLM(ctx context.Context, from, to time.Time, filter domain.MetricsFilter) ([]domain.LLMUsage, error) {
	rows, err := statements(s.db).Select("execution_id", "chat_run_id", "pipeline_id", "node_type", "origin", "provider_id", "provider_name", "provider_kind", "model", "succeeded", "tokens_reported", "prompt_tokens", "completion_tokens", "queue_wait_ms", "duration_ms", "estimated_cost_usd", "occurred_at").From("metric_llm_events").Where("occurred_at >= ?", stamp(from)).Where("occurred_at <= ?", stamp(to)).OrderBy("occurred_at").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricActivity(ctx context.Context, from, to time.Time) ([]domain.MetricActivityEvent, error) {
	rows, err := statements(s.db).Select("kind", "outcome", "duration_ms", "occurred_at").From("metric_activity_events").Where("occurred_at >= ?", stamp(from)).Where("occurred_at <= ?", stamp(to)).OrderBy("occurred_at").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricResources(ctx context.Context, from, to time.Time) ([]domain.MetricsResourcePoint, error) {
	rows, err := statements(s.db).Select("process_name", "cpu_percent", "working_set_bytes", "occurred_at").From("metric_resource_samples").Where("occurred_at >= ?", stamp(from)).Where("occurred_at <= ?", stamp(to)).OrderBy("occurred_at").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricExecutionRollups(ctx context.Context, from, to time.Time, filter domain.MetricsFilter) ([]MetricExecutionRollup, error) {
	rows, err := statements(s.db).Select("bucket", "pipeline_id", "pipeline_name", "trigger_kind", "status", "run_count", "duration_sum_ms", "queue_wait_sum_ms", "node_count", "failed_node_count").From("metric_execution_rollups").Where("bucket >= ?", from.Format("2006-01-02")).Where("bucket <= ?", to.Format("2006-01-02")).OrderBy("bucket").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricLLMRollups(ctx context.Context, from, to time.Time, filter domain.MetricsFilter) ([]MetricLLMRollup, error) {
	rows, err := statements(s.db).Select("bucket", "provider_id", "provider_name", "provider_kind", "model", "succeeded", "call_count", "prompt_tokens", "completion_tokens", "tokens_reported_count", "duration_sum_ms", "queue_wait_sum_ms", "estimated_cost_sum_usd", "priced_count").From("metric_llm_rollups").Where("bucket >= ?", from.Format("2006-01-02")).Where("bucket <= ?", to.Format("2006-01-02")).OrderBy("bucket").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricActivityRollups(ctx context.Context, from, to time.Time) ([]MetricActivityRollup, error) {
	rows, err := statements(s.db).Select("bucket", "kind", "outcome", "event_count", "duration_sum_ms").From("metric_activity_rollups").Where("bucket >= ?", from.Format("2006-01-02")).Where("bucket <= ?", to.Format("2006-01-02")).OrderBy("bucket").QueryContext(ctx)
	if err != nil {
...
func (s *Store) readMetricResourceRollups(ctx context.Context, from, to time.Time) ([]MetricResourceRollup, error) {
	rows, err := statements(s.db).Select("bucket", "process_name", "sample_count", "cpu_sum", "cpu_peak", "working_set_sum", "working_set_peak").From("metric_resource_rollups").Where("bucket >= ?", from.Format("2006-01-02")).Where("bucket <= ?", to.Format("2006-01-02")).OrderBy("bucket").QueryContext(ctx)
	if err != nil {
