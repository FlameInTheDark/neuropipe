...
database. Add it from the KV Store category of the node library. Channels can
be listed explicitly or matched with Redis glob patterns such as `events:*`.
Because delivery is unattended, the trigger follows the publishing trust
model: the pipeline revision must be trusted and the binding enabled before
messages start pipelines.
