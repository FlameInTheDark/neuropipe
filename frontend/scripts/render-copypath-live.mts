/* Builds the copy-path live test page: bundles scripts/copy-path-live-entry.tsx
   to an IIFE with esbuild (real react-dom/client — no SSR portal shim), then
   inlines the JS and the app's compiled CSS (from the single-file dist build)
   into one standalone HTML file.
   Output: /home/z/my-project/scripts/jsonviewer-copypath-live.html */
...
const result = await build({
  entryPoints: [path.resolve(import.meta.dirname, "copy-path-live-entry.tsx")],
  bundle: true,
  platform: "browser",
  format: "iife",
  write: false,
...
<title>JsonViewerModal copy-path live test</title>
<style>${styles}</style>
</head>
...
writeFileSync("/home/z/my-project/scripts/jsonviewer-copypath-live.html", page);
console.log("wrote /home/z/my-project/scripts/jsonviewer-copypath-live.html");
