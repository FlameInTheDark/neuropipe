  ...
  return <section className="flex h-full min-h-0 flex-col"><PageHeader title={t('chat.title')} description={t('chat.description')} actions={<>
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[15.5rem_minmax(0,1fr)]">
      <aside className="min-h-0 border-r border-zinc-800 bg-zinc-950/35 p-3"><div className="mb-3 flex items-center gap-2">{mode === 'model' ? <Button className="flex-1" size="sm" onClick={() => void create('model')}><Plus className="size-3.5" />{t('chat.new')}</Button> : <div className="relative flex-1"><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('chat.findPipeline')} className="h-7 text-xs" /></div>}</div>
       ...
       <main className="flex min-w-0 min-h-0 flex-col">{loading ? <div className="flex h-full items-center justify-center text-sm text-zinc-500"><Loader2 className="mr-2 size-4 animate-spin" />{t('chat.loading')}</div> : !selected ? <EmptyState variant="plain" icon={MessageCircle} title={mode === 'model' ? t('chat.startModel') : t('chat.choosePipeline')} description={mode === 'model' ? t('chat.startModelDescription') : t('chat.choosePipelineDescription')} action={mode === 'model' ? { label: t('chat.new'), onClick: () => void create('model') } : undefined} /> : <><div ref={feedRef} className="muted-scroll min-h-0 flex-1 overflow-y-auto"><div className="mx-auto max-w-3xl px-6 py-7">{messages.map((message) => {
