...
function MarkdownPreview({ text }: { text: string }) {
  const { t } = useTranslation();
  if (!text.trim()) {
    return <p className="text-[13px] italic text-ink-500">{t("textEditor.emptyPreview")}</p>;
  }

  return (
    <div className="mx-auto max-w-[740px] text-[13.5px] leading-[1.7] text-ink-200">
      <ReactMarkdown
          ...
          input: (p) =>
            p.type === "checkbox" ? (
              <input
                ...
                disabled
                checked={p.checked}
                className="mr-1.5 h-3 w-3 translate-y-[1px] accent-ink-500"
              />
            ) : (
              <input {...p} />
            ),
          img: (p) => <img className="my-3 max-w-full rounded-lg border border-ink-700" {...p} />,
        }}
      >
        {text}
      </ReactMarkdown>
