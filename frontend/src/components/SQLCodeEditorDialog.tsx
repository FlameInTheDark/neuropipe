...
const reservedParameterIDs = new Set(["sql"]);

// EditableParameter wraps SQLParameter with a stable internal clientId used as
// the React key. The clientId is generated once when the parameter enters the
// editor and never changes while the user types — this prevents the row from
// remounting (and losing input focus) when the user edits the PIN ID field,
// which is the user-editable value that also becomes item.id.
interface EditableParameter extends SQLParameter {
  clientId: string;
}

function newClientId(): string {
  // crypto.randomUUID is available in the Wails webview; fall back to a counter
  // when running in plain browsers without it (unit tests, storybooks, etc.).
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") {
    return crypto.randomUUID();
  }
  return `param-${Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`;
}

function parametersFromConfig(value: unknown): EditableParameter[] {
...
function nextParameter(parameters: readonly EditableParameter[]): EditableParameter {
  const ids = new Set(parameters.map((item) => item.id));
  let index = parameters.length + 1;
  while (ids.has(`parameter${index}`)) index += 1;
  return { clientId: newClientId(), id: `parameter${index}`, name: `parameter${index}`, label: `parameter${index}`, type: { kind: "any" }, required: false };
}
...
export function SQLCodeEditorDialog({ open, config, onClose, onSave }: { open: boolean; config: Record<string, unknown>; onClose: () => void; onSave: (config: Record<string, unknown>) => void }) {
  ...
  const [parameters, setParameters] = useState<EditableParameter[]>(() => parametersFromConfig(config.parameters));
  ...
  const run = async () => {
    ...
    try {
      setResult(await desktop.debugDatabase({ databaseId, sql, parameters: parameters.map((item) => ({ name: item.name, value: debugValue(debugValues[item.clientId] ?? "", item.type.kind) })), maxRows: 200 }));
      await refreshSchema();
    }
  ...
  const parameterError = (() => {
    ...
    for (const parameter of parameters) {
      ...
      if (reservedParameterIDs.has(parameter.id) || reservedParameterIDs.has(parameter.name)) return t("sql.reservedParameter");
